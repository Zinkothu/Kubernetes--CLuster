...
kind create cluster --config ../kindconfig/kindconfig-v134-jpdc-cluster.yaml
sleep 1
kubectl apply -f https://raw.githubusercontent.com/metallb/metallb/v0.15.3/config/manifests/metallb-native.yaml --context kind-jpdc-cluster
pods_checking=false
...
while ! $pods_checking; do
    ...
    if kubectl get pods -n metallb-system | grep -i controller | grep -q 'Running' && kubectl get pods -n metallb-system | grep -i speaker | grep -q 'Running' ; then
        ...
        kubectl create -f ../kindconfig/metallb-v0153-ipaddress-pool-2.yaml --context kind-jpdc-cluster
...
sleep 1
kubectl config rename-context kind-jpdc-cluster jpdc-cluster
sleep 1
license=$(cat consul-ent-license.hclic)
kubectl create ns consul --context jpdc-cluster

kubectl create secret generic consul-ent-license --from-literal="key=${license}" -n consul --context jpdc-cluster

watch kubectl get secrets consul-ent-license -n consul -o yaml --context jpdc-cluster
# watch kubectl get pods -A --context jpdc-cluster
